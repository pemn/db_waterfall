...
def pd_waterfall_compare(vs, names, output, display):
...
def xl_waterfall_compare(vs, names, output, display, title = ''):
...
def db_waterfall(input_a, condition_a, groups_a, value_a, input_b, condition_b, groups_b, value_b, output, display):
  ...
  if output.lower().endswith('xlsx'):
    xl_waterfall_compare([pt_a.squeeze(), pt_b.squeeze()], [name_a, name_b], output, display, '%sⅹ%s' % (name_a, name_b))
  else:
    pd_waterfall_compare([pt_a.squeeze(), pt_b.squeeze()], [name_a, name_b], output, display)
